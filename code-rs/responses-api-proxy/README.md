...
A privileged user (i.e., `root` or a user with `sudo`) who has access to `OPENAI_API_KEY` would run the following to start the server, as `code-responses-api-proxy` reads the auth token from `stdin`:

```shell
printenv OPENAI_API_KEY | code-responses-api-proxy --http-shutdown --server-info /tmp/server-info.json
```
...
code exec -c "model_providers.openai-proxy={ name = 'OpenAI Proxy', base_url = '${PROXY_BASE_URL}/v1', wire_api='responses' }" \
...
- Reads the API key from `stdin`. All callers should pipe the key in (for example, `printenv OPENAI_API_KEY | code-responses-api-proxy`).
...
```
code-responses-api-proxy [--port <PORT>] [--server-info <FILE>] [--http-shutdown]
```
...
- We leverage [`code_process_hardening`](https://github.com/tytsxai/beacon-code/blob/main/code-rs/process-hardening/README.md) so `code-responses-api-proxy` is run with standard process-hardening techniques.
