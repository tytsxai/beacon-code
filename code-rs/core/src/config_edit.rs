...
fn append_agent_entry(
    ...
    if let Some(desc) = description
        && !desc.trim().is_empty()
    {
        t["description"] = toml_edit::value(desc.to_string());
    }
    if let Some(cmd) = command
        && !cmd.trim().is_empty()
    {
...
mod tests {
    ...
    async fn set_default_model_and_effort_top_level_when_no_profile() {
        ...
        assert_eq!(
            table.get(CONFIG_KEY_MODEL).and_then(|value| value.as_str()),
            Some("gpt-5.1-code")
        );
        assert_eq!(
            table
    ...
    async fn persist_overrides_creates_nested_tables() {
        ...
        persist_overrides(
            ...
            &[
                (&["a", "b", "c"], "v"),
                (&["x"], "y"),
                (&["profiles", "p1", CONFIG_KEY_MODEL], "gpt-5.1-code"),
            ],
        )
        .await
        .expect("persist");
        ...
        let parsed: toml::Value = toml::from_str(&contents).expect("valid toml");
        let table = parsed.as_table().expect("root table");
        assert_eq!(table.get("x").and_then(toml::Value::as_str), Some("y"));
        let a_table = table
            .get("a")
...
"#;
    ...
    async fn changing_only_effort_preserves_existing_model_top_level() {
        ...
        let contents = read_config(code_home).await;
        let expected = r#"model = "gpt-5.1-code"
model_reasoning_effort = "high"
"#;
        assert_eq!(contents, expected);
    }
...
"#;
    ...
    async fn persist_non_null_skips_none_top_level() {
        ...
        persist_non_null_overrides(
            ...
            None,
            &[
                (&[CONFIG_KEY_MODEL], Some("gpt-5.1-code")),
    ...
    async fn persist_non_null_respects_profile_override() {
        ...
        let team = profiles
            ...
            .expect("team profile");
        assert_eq!(
            team.get(CONFIG_KEY_MODEL).and_then(|value| value.as_str()),
            Some("o3")
        );
