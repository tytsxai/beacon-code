...
impl BrowserManager {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        let user_data_path = if let Some(dir) = &config.user_data_dir {
            ...
            let timestamp = std::time::SystemTime::now()
                ...
                .unwrap_or_default()
                .as_millis();
            let temp_path = format!("/tmp/code-browser-{}-{timestamp}", std::process::id());
        ...
        let cdp_page = if config.connect_port.is_some() || config.connect_ws.is_some() {
            ...
            if !pages.is_empty() {
                ...
                if let Some(page) = active_page {
                    ...
                    page
                } else if let Some(page) = last_allowed {
    ...
    pub fn get_status_sync(&self) -> String {
        ...
        let viewport_width = cfg.1;
        let viewport_height = cfg.2;
    ...
    async fn capture_screenshot_regular(&self) -> Result<(Vec<std::path::PathBuf>, String)> {
        ...
        let assets = assets_guard.as_ref().cloned().ok_or_else(|| {
            BrowserError::AssetError("Assets manager was unexpectedly missing".to_string())
        })?;
        drop(assets_guard);
