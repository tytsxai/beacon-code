...
pub enum Command {
    /// Submit a new Beacon Code Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Beacon Code Cloud task.
    Status(StatusCommand),
    /// Apply the diff for a Beacon Code Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Beacon Code Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Beacon Code Cloud.
    ...
    /// Git branch to run in Beacon Code Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Beacon Code Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Beacon Code Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Beacon Code Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
