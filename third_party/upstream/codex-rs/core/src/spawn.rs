...
///
/// 1. The process was spawned by Beacon Code as part of a shell tool call.
...
pub(crate) async fn spawn_child_async(
    ...
    // If this Beacon Code process dies (including being killed via SIGKILL), we want
    ...
    unsafe {
        ...
        cmd.pre_exec(move || {
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                // run _after_ the parent (i.e., the Beacon Code process) has already
                // exited, then parent will be the closest configured "subreaper"
                // ancestor process, or PID 1 (init). If the Beacon Code process has exited
