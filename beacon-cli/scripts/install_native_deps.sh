...
# Install native runtime dependencies for beacon-cli.
#
# Usage
#   install_native_deps.sh [--workflow-url URL] [BEACON_CLI_ROOT]
#
...
if [ -n "$BEACON_CLI_ROOT" ]; then
  # The caller supplied a release root directory.
  BIN_DIR="$BEACON_CLI_ROOT/bin"
else
  ...
  BEACON_CLI_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
  BIN_DIR="$BEACON_CLI_ROOT/bin"
fi
