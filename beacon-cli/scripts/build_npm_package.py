...
RESPONSES_API_PROXY_NPM_ROOT = REPO_ROOT / "code-rs" / "responses-api-proxy" / "npm"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build or stage the Beacon Code CLI npm package.")
    parser.add_argument(
        "--package",
        choices=("beacon-code", "beacon-code-responses-api-proxy", "beacon-code-sdk"),
        default="beacon-code",
        help="Which npm package to stage (default: beacon-code).",
    )
    parser.add_argument(
        "--version",
        ...
        if release_version:
            ...
            if package == "beacon-code":
                print(
                    ...
                    "Verify the CLI:\n"
                    f"    node {staging_dir_str}/bin/beacon.js --version\n"
                    f"    node {staging_dir_str}/bin/beacon.js --help\n\n"
                )
            elif package == "beacon-code-responses-api-proxy":
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
                    "Verify the responses API proxy:\n"
                    f"    node {staging_dir_str}/bin/code-responses-api-proxy.js --help\n\n"
                )
            else:
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="beacon-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    if package == "beacon-code":
        ...
        shutil.copy2(BEACON_CLI_ROOT / "bin" / "beacon.js", bin_dir / "beacon.js")
        shutil.copy2(BEACON_CLI_ROOT / "bin" / "coder.js", bin_dir / "coder.js")
        ...
        package_json_path = BEACON_CLI_ROOT / "package.json"
    elif package == "beacon-code-responses-api-proxy":
        bin_dir = staging_dir / "bin"
        bin_dir.mkdir(parents=True, exist_ok=True)
        launcher_src = RESPONSES_API_PROXY_NPM_ROOT / "bin" / "code-responses-api-proxy.js"
        shutil.copy2(launcher_src, bin_dir / "code-responses-api-proxy.js")
    ...
    elif package == "beacon-code-sdk":
        package_json_path = SDK_ROOT / "package.json"
        stage_sdk_sources(staging_dir)
    else:
...
def stage_sdk_sources(staging_dir: Path) -> None:
    ...
    if not dist_src.exists():
        raise RuntimeError("beacon-code-sdk build did not produce a dist directory.")
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="beacon-npm-pack-") as pack_dir_str:
