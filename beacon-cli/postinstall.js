...
export async function runPostinstall(options = {}) {
    ...
    try {
      ...
      if (resolved) {
        ...
        if (!looksLikeOurs) {
          ...
          console.warn(
            "[notice] We will still install Beacon Code, available as `beacon` (primary) and `coder`.",
          );
          console.warn(
            "         If `code` runs another tool, prefer using: beacon",
          );
          console.warn(
  ...
  if (isGlobal && !isNpx)
    try {
      ...
      const installedCmds = new Set(["beacon", "coder"]); // global install exposes beacon + coder via package manager
      const skippedCmds = [];
      ...
      if (isBun) {
        ...
        if (other && existsSync(bunShim)) {
          ...
          // No conflict: create a wrapper that forwards to `beacon`
          try {
            ...
            if (isWindows) {
              ...
              const content = `#!/bin/sh\nexec "$(dirname \"$0\")/beacon" "$@"\n`;
              writeFileSync(wrapperPath, content);
              chmodSync(wrapperPath, 0o755);
            }
            console.log("✓ Created 'code' wrapper -> beacon (bun)");
        ...
        if (installedCmds.has("code")) {
          console.log("Use 'code' or 'beacon' to launch Beacon Code.");
        } else {
          console.log("Use 'beacon' to launch Beacon Code.");
        }
      } else {
        // npm/pnpm/yarn path
        ...
        const ensureWrapper = (name, args) => {
          ...
          try {
            ...
            if (isWindows) {
              const content = `@echo off\r\n"%~dp0${collision ? "beacon" : "code"}" ${args} %*\r\n`;
              writeFileSync(wrapperPath, content);
            } else {
              const content = `#!/bin/sh\nexec "$(dirname \"$0\")/${collision ? "beacon" : "code"}" ${args} "$@"\n`;
              writeFileSync(wrapperPath, content);
              chmodSync(wrapperPath, 0o755);
            }
            console.log(
              `✓ Created wrapper '${name}' -> ${collision ? "beacon" : "code"} ${args}`,
            );
        ...
        if (collision) {
          ...
          // No collision; ensure a 'code' wrapper exists forwarding to 'beacon'
          if (globalBin) {
            try {
              ...
              console.log("✓ Created 'code' wrapper -> beacon");
              installedCmds.add("code");
            } catch (e) {
