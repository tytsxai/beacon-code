...
要让 Zed 连接 Beacon Code（Code）的 ACP 服务器，在 `settings.json` 中添加：
...
```jsonc
{
  "agent_servers": {
    "Code": {
      "command": "npx",
      "args": ["-y", "@tytsxai/beacon-code", "acp"]
    }
  }
}
```
...
- Zed Stable `0.201.5`（2025 年 8 月 27 日发布）或更新版本在 Agent Panel 中加入 ACP 支持。接入 Beacon Code 前请通过 `Zed → Check for Updates` 升级。Zed 文档说明 ACP 是驱动外部智能体 CLI 的机制。
- 外部智能体位于 Agent Panel（`cmd-?`）内。点击 `+` 新建线程，从外部智能体列表选择 `Code`（Beacon Code）。Zed 以子进程方式通过 JSON‑RPC 运行我们的 CLI，提示与 diff 预览都保留在本地。
- Zed 会按条目自动安装依赖。如果保持 `command = "npx"`，Zed 会在首次触发集成时下载发布的 `@tytsxai/beacon-code` 包。

## Beacon Code 如何实现 ACP
...
- MCP 配置集中在 `CODE_HOME/config.toml`。使用 `[experimental_client_tools]` 可将文件读写和权限请求委托给 Zed，让其 UI 处理审批。最小示例如下：
...
添加 Code（Beacon Code）智能体时 Zed 会自动连接这些工具，上述标识符与默认值一致。
- CLI 入口（`npx @tytsxai/beacon-code acp`）是对 Rust 二进制（`cargo run -p code-mcp-server -- --stdio`）的薄封装，随 Beacon Code 一同发布。从源码构建时可将 `command` 换成该二进制的绝对路径。
