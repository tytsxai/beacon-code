...
- **代理**：启动前确保 shell 已设置 `http_proxy`/`https_proxy`/`ALL_PROXY`，并在 Beacon 需要联网时保持有效。
...
- 配置文件位置：`~/.code/config.toml`。示例见 `config-examples.md`。
...
- **模型不可用/未登录**：确认 ChatGPT 登录或 `OPENAI_API_KEY` 设置，并检查配置是否生效。
- **网络超时**：确认代理/镜像有效，必要时重试或切换网络。

## 贡献

- 本地化改动集中在 `docs/zh/` 和后续的字符串资源；保持英文版不改名/不重排，便于维护。
