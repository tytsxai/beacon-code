...
Code configuration gives you fine-grained control over the model, execution environment, and integrations available to the CLI. Use this guide alongside the workflows in [`code exec`](./exec.md), the guardrails in [Sandbox & approvals](./sandbox.md), and project guidance from [AGENTS.md discovery](./agents_md.md).
...
Code supports several mechanisms for setting config values:
...
- The `$CODE_HOME/config.toml` configuration file (default: `~/.code`).

## Config files and managed layers

Configuration is loaded in layers; higher layers override lower layers.

Layers (top overrides bottom):

1. **Managed preferences** (macOS only; set via device management profiles)
2. **Managed config file**:
   - Unix: `/etc/code/managed_config.toml`
   - Non-Unix: `$CODE_HOME/managed_config.toml`
3. **User config file**: `$CODE_HOME/config.toml`

Practical implications:

- Managed layers are intended for admins and can enforce org policy (models, endpoints, sandbox defaults, etc).
- If a value “won’t change” even after editing `config.toml`, check the managed layers first.

Both the `--config` flag and the `config.toml` file support the following options:

## Feature flags

Optional and experimental capabilities are toggled via the `[features]` table in `$CODE_HOME/config.toml`. If you see a deprecation notice mentioning a legacy key (for example `experimental_use_exec_command_tool`), move the setting into `[features]` or pass `--enable <feature>`.
...
This option lets you add to the default set of model providers bundled with Beacon. The map key becomes the value you use with `model_provider` to select the provider.
...
Export your key before launching Beacon: `export AZURE_OPENAI_API_KEY=…`
...
Number of times Beacon will attempt to reconnect when a streaming response is interrupted. Defaults to `5`.
...
If the selected model is known to support reasoning (for example: `o3`, `o4-mini`, `codex-*`, `gpt-5.1-code-max`, `gpt-5.1`, `gpt-5.1-code`, `gpt-5.2`), reasoning is enabled by default when using the Responses API. As explained in the [OpenAI Platform documentation](https://platform.openai.com/docs/guides/reasoning?api-mode=responses#get-started-with-reasoning), this can be set to:

- `"minimal"`
- `"low"`
- `"medium"` (default)
- `"high"`
- `"xhigh"` (available on `gpt-5.1-code-max` and `gpt-5.2`)
...
Specifies the default OSS provider to use when running Beacon. This is used when the `--oss` flag is provided without a specific provider.
...
On macOS (and soon Linux), all writable roots (including `cwd`) that contain a `.git/` or `.code/` folder _as an immediate child_ will configure those folders to be read-only while the rest of the root stays writable. This means that commands like `git commit` will fail, by default (as it entails writing to `.git/`), and will require Beacon to ask for permission.
...
```toml
[tools]
web_search = true   # allow Beacon to issue first-party web searches without prompting you (deprecated)
view_image = false  # disable image uploads (they're enabled by default)
```

The `view_image` toggle is useful when you want to include screenshots or diagrams from your repo without pasting them manually. Beacon still respects sandboxing: it can only attach files inside the workspace roots you allow.
...
You can configure Beacon to use [MCP servers](https://modelcontextprotocol.io/about) to give Beacon access to external applications, resources, or services.
...
[Streamable HTTP servers](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#streamable-http) enable Beacon to talk to resources that are accessed via a http url (either on localhost or another domain).
...
After enabling it, run `code mcp login <server-name>` when the server supports OAuth.
...
When both `enabled_tools` and `disabled_tools` are specified, Beacon first restricts the server to the allow-list and then removes any tools that appear in the deny-list.
...
There is an ever growing list of useful MCP servers that can be helpful while you are working with Beacon.
...
- [Sentry](https://docs.sentry.io/product/sentry-mcp/) — access to your Sentry logs
...
Beacon can emit [OpenTelemetry](https://opentelemetry.io/) **log events** that
...
With OTEL enabled Beacon emits the following event types (in addition to the
metadata above):
...
or enable mutual TLS. Relative paths are resolved against `CODE_HOME/`:
...
If you build Beacon from source the OTEL crate is still behind an `otel` feature
...
`"thread-id"` contains a string that identifies the Beacon session that produced the notification; you can use it to correlate multiple turns that belong to the same task.
...
To have Beacon use this script for notifications, you would configure it via `notify` in `~/.code/config.toml` using the appropriate path to `notify.py` on your computer:
...
By default, Code records messages sent to the model in `$CODE_HOME/history.jsonl`. Note that on UNIX, the file permissions are set to `o600`, so it should only be readable and writable by the owner.
...
count. When the file grows beyond the limit, Beacon removes the oldest entries,
...
> See also [AGENTS.md discovery](./agents_md.md) for how Beacon locates these files during a session.
...
> [!NOTE]
> Beacon emits desktop notifications using terminal escape codes. Not all terminals support these (notably, macOS Terminal.app and VS Code's terminal do not support custom notifications. iTerm2, Ghostty and WezTerm do support these notifications).
...
To force users on a given machine to use a specific login method or workspace, use managed configurations along with either or both of the following fields:
...
# flow and the id will be validated during the oauth callback as well as every time Beacon starts.
forced_chatgpt_workspace_id = "00000000-0000-0000-0000-000000000000"
```

If the active credentials don't match the config, the user will be logged out and Beacon will exit.
...
| ------------------------------------------------ | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `model`                                          | string                                                            | Model to use (e.g., `gpt-5.1-code-max`).                                                                                       |
...
| `mcp_servers.<id>.enabled`                       | boolean                                                           | When false, Beacon skips starting the server (default: true).                                                                    |
