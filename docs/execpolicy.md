...
Beacon can enforce your own rules-based execution policy before it runs shell commands. Policies live in `.rules` files under `~/.code/rules`.
...
1. Create a policy directory: `mkdir -p ~/.code/rules`.
2. Add one or more `.rules` files in that folder. Beacon automatically loads every `.rules` file in there on startup.
...
- `decision` sets the severity; Beacon picks the strictest decision when multiple rules match (forbidden > prompt > allow).
- `match` and `not_match` act as optional unit tests. Beacon validates them when it loads your policy, so you get feedback if an example has unexpected behavior.

In this example rule, if Beacon wants to run commands with the prefix `git push` or `git fetch`, it will first ask for user approval.
