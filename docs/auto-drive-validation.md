...
| 配置与默认：配置在 `config.toml [auto_drive]`（model、use_chat_model、continue_mode、review/agents/QA/cross-check 默认开启，observer_cadence 等）；`config.toml.example` 展示默认 gpt-5.1 + 高推理，continue_mode=ten-seconds。 | `docs/auto-drive.md:105-108`，`config.toml.example:23-47`，`code-rs/core/src/config_types.rs:773-900` | 默认值与字段位置一致；`use_chat_model` 为顶层/设置项；默认模型 `gpt-5.1`，reasoning `high`，continue_mode `ten-seconds`。 | 准确 | 无需修改。 |
...
配置与默认：配置位于 `config.toml` 顶层与 `[auto_drive]`（model/use_chat_model/continue_mode/observer/review/agents/QA/cross-check 等）；示例默认 `gpt-5.1` + reasoning `high`，continue_mode=`ten-seconds`。
